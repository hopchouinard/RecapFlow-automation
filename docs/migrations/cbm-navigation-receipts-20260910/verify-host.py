...
root=pathlib.Path('/srv/community-brain');backup=root/'artifacts/cbm-navigation-20260910'
def manifest(p):
 d={}
 ...
 return d
assert {n:manifest(root/n) for n in ['corpus','config','files','meeting-archive-20260910']}==json.loads((backup/'data-manifest-before.json').read_text())
...
assert mounts.pop('/app/web/dist')==('/srv/community-brain/workspaces/cbm-navigation-20260910/dist',False);oldmounts.pop('/app/web/dist');assert mounts==oldmounts
