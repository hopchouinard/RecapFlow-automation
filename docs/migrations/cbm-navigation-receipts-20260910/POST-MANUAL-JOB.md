...
## Active frontend and human grant recovery (request010)

Include `/srv/community-brain/workspaces/cbm-navigation-20260910/`, its
`effective-frontend-manifest.json`, the current manual overlay/runtime manifest
and installed Mac renderer in the recovery inventory. The frontend pin is
`7b2f6cebf4f15aa7e33aaae3c77bf99e9d57682edc60535aa64fbd76537e0bb5`.
Retain the request009 source/archive packet and its original pins as well.

The Community Brain Authentik scope mapping
`962deddf-1792-4e27-9e01-b0c9249dae2b`, exclusively bound to provider8, preserves
operator-group read permissions and adds only jobs:submit/sources:upload when the
active human account matches established account ID6 and its pinned UUID.
Recovery must preserve that exact predicate and the existing app group binding;
never broaden it to all members or substitute a display-name match. Include the
private before/after mapping and policy snapshots from request010's off-host
controls archive. Restore only the scoped expression when explicitly reverting
this grant, not unrelated provider settings, service identities or application data.
Human sign-in verification remains separate from service-token probes.
See ../navigation/receipt.md for current manifest hashes and private recovery paths.
