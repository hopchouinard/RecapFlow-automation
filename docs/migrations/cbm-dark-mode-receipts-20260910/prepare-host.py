"""Request008: preserve current controls and prepare one API-only static mount."""
import pathlib,hashlib,json,subprocess,os,shutil,datetime
os.umask(0o077)
root=pathlib.Path('/srv/community-brain');base=root/'workspaces/manual-20260910';ui=root/'workspaces/cbm-dark-mode-20260910';backup=root/'artifacts/cbm-dark-mode-20260910';backup.mkdir(mode=0o700)
...
old=manifest(backup/'old-dist');new=json.loads((ui/'frontend-manifest.json').read_text());assert manifest(ui/'dist')==new
...
new_overlay=old_overlay+'      - /srv/community-brain/workspaces/cbm-dark-mode-20260910/dist:/app/web/dist:ro\n'
def atomic(p,s):
 t=p.with_name('.'+p.name+'.008.tmp');t.write_text(s);t.chmod(0o600);t.replace(p)
