...
import pathlib,json,subprocess,datetime,os,sys
...
def apple(code):return subprocess.check_output(['osascript','-e',code],text=True,timeout=15).strip()
...
if __name__=='__main__':
    try:main()
    except Exception:raise SystemExit('Legacy intake state unconfirmed; no automatic resume. Inspect management status.') from None
