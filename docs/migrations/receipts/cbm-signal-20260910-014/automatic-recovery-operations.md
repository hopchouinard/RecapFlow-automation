...
`/srv/community-brain/workspaces/cbm-signal-20260910-effective/dist`.
...
## Request014 Signal theme

The approved Signal styling was deployed2026-09-10 at23:43 UTC using the same
quiet lease and Mac scheduler lock. The effective static directory retains all
prior hashed assets (14 files total). JavaScript bytes match request012 exactly.
Runtime renderer and recovery inventory now pin/include the Signal frontend.

Private controls are on VM109 at
`/srv/community-brain/artifacts/cbm-signal-20260910-014/{before,after}/`, on the Mac
at `~/.local/state/community-brain-management/signal-014/{before,after}/`, and
verified off-host on platform-db at
`/var/backups/community-brain/signal-deployment-014-{before,after}/`.
For a Signal-only rollback, use this before pair, restoring the preview012 static
path and matching renderer/runtime pins under the ordered locks, then render
fresh Infisical identities. Preserve backend, automatic controls and all data.
See `../signal/receipt.md` for current hashes and acceptance evidence.
