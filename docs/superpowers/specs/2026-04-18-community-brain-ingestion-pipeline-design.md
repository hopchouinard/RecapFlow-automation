...
6. **Vector search under-weights rare entity tokens.** Pure-vector retrieval via `nomic-embed-text` favors thematic similarity ("commit", "outcomes", "decisions") over proper-noun matches ("Adam", "Gold Flamingo"). Entity-grounded queries (*"What did Adam from Gold Flamingo commit to?"*) failed to surface the entity's actual chunks even when those chunks ranked at 0.42+ similarity for keyword-rephrased versions of the same question. **Addressed in v2:** see [`docs/superpowers/specs/2026-04-27-hybrid-retrieval-v2-design.md`](2026-04-27-hybrid-retrieval-v2-design.md). v2 ships hybrid retrieval (vector + BM25 RRF) on the existing `/query` endpoint with no new parameters; the BM25 leg surfaces entity-grounded chunks the pure-vector leg missed.

7. **Vector search ignores structured metadata flags.** Pure-vector retrieval matches on semantic content of the question text, not on the structured fields Stage C populates. For *"What questions came up across these calls that didn't get fully answered?"* (which is exactly what `has_unresolved_question=True` is supposed to answer), retrieval surfaced 1 of 38 tagged chunks in the corpus — the rest were `signal:general` overviews that thematically mention questions but aren't actually tagged. The same pattern would affect queries that should naturally pivot off `decisions`, `action_items`, `references_prior`, `has_question`, or `has_answer`. **Addressed in v2:** see [`docs/superpowers/specs/2026-04-27-hybrid-retrieval-v2-design.md`](2026-04-27-hybrid-retrieval-v2-design.md). v2 introduces a cue-driven metadata-aware boost layer (post-RRF, pre-truncation) that adds additive deltas to chunks whose structured flags align with question-side lexical cues — without LLM intent classification or new API parameters. Findings 6 and 7 share the same root: pure vector similarity was the wrong primary ranking signal when the schema has rich structured fields purpose-built for these query types.
...
- **Missing / unresolved questions** — corpus has rich material (38 chunks tagged `has_unresolved_question=True` across the 8-session validation set, distributed: 0 / 11 / 8 / 1 / 4 / 7 / 4 / 3). **But pure vector retrieval surfaced only 1 of 38 tagged chunks** for the natural-language query — the rest of the top-10 were signal:general overviews that *talk about* questions thematically without being flagged. Validated answer was internally faithful (no hallucination) but structurally under-represented the corpus. Same root cause as finding 6: structured metadata isn't leveraged by ranking. Hybrid retrieval (vector + metadata filter) would solve this.
