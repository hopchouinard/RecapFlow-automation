...
script = Path(__file__).resolve().with_name("weekly_retrieval.py")
result = subprocess.run(
    [
        ...
        "512m",
        "-v",
        str(script) + ":/checks/weekly_retrieval.py:ro",
        "community-brain:rehearsal",
        "python",
        "/checks/weekly_retrieval.py",
    ],
    input=json.dumps(
...
(root / "weekly-retrieval-result.json").write_text(
