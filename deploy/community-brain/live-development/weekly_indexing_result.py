...
assert len(rows) == 30 and table.count_rows() == 30
...
journal = [
    json.loads(x)
    for x in Path("/state/files/weekly-indexing-calls.jsonl").read_text().splitlines()
]
assert sum(r["state"] == "intent" for r in journal) == 31
assert sum(r["state"] == "response" for r in journal) == 31
result.update(
    requests=31,
    responses=31,
