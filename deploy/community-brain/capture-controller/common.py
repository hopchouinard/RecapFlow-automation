...
sys.path.insert(0,str(Path(__file__).parent/'forge-core'))
import recovery as r
ROOT=Path('/srv/dev-data/workspaces/cbm-capture-controller-20260921-031')
...
def load_spec(path,expected,check_time=True):
    ...
    s=json.loads(raw)
    required={'schema','scope','owner_id','operation_id','capture_id','packet','packet_sha256','operation','fixture','locks','holds','hold_bindings','incumbent','database','deadlines','exercise'}
    ...
    if s['exercise'] not in ('normal','hold_for_expiry','wait_for_interrupt','wait_for_fence_loss'):raise ValueError('unknown exercise')
    ...
    for key in ('source','restore'):
        ...
        if not identity['name'].startswith('cbm-r031-pg-') or row['Id']!=identity['id'] or row['Name'].lstrip('/')!=identity['name'] or row['Image']!=identity['image'] or row['HostConfig']['NetworkMode']!='none' or row['HostConfig']['PortBindings']:raise ValueError('foreign database')
    if not s['incumbent']['name'].startswith('cbm-r031-inc-'):raise ValueError('foreign incumbent')
    return s
