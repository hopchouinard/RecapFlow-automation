...
def main(mode):
 ...
 with (LOCAL/'isolated-scheduler.lock').open('a') as lock:
  ...
  with LeaseSupervisor(holder,job,nonce,interval=3,reply_timeout=20) as lease:
   adapter=Adapter(lease,identity)
   if mode in ('disable-signup','disable-signup-readback','validate-postbrowser-v2','inspect','data-inspect'):
    result=adapter.remote(mode,{},timeout=900 if mode=='validate-postbrowser-v2' else 240)
   else:values=adapter.read()
   if mode in ('disable-signup','disable-signup-readback','validate-postbrowser-v2','inspect','data-inspect'):pass
   elif mode=='setup':
   ...
   elif mode not in ('inventory','renewal','renewal-resume'):
    result=adapter.remote(mode,values,timeout=240)
    if mode=='setup-resume' and not (LOCAL/'dev-collector.json').exists():adapter.collector(values)
