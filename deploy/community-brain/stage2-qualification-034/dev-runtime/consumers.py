...
def setup(values):
 ...
 config={'global':{'scrape_interval':'5s'},'scrape_configs':[{'job_name':'cbm-request034-api','metrics_path':'/metrics','authorization':{'credentials_file':'/etc/prometheus/metrics-token'},'static_configs':[{'targets':['api:8090']}]}]}
 ...
 atomic(PRIVATE/'kuma-admin.json',json.dumps({'username':'request034','password':password}))
...
def verify(payload):
 ...
 while True:
  ...
  rows=[x for x in data['data']['activeTargets'] if x['scrapePool']=='cbm-request034-api']
