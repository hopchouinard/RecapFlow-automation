"""Bounded validation on disposable Request034 state. Never production."""
...
def submit(values,date,fathom=False):
 ...
 token=values['CB_PROD_MANUAL_OPERATOR_TOKEN'];meeting='request034-'+date
 sources={}
...
def manual_after_reconciliation(_):
 # The first Request034 attempt was correctly refused by the actual API's
 # quiet lease. Preserve that intent. This is a fresh, separately named
 # attempt after disposable boot reconciliation, with no management lease.
 import shlex
 marker=ROOT/'manual-open-validation-intent'
 assert not marker.exists();marker.touch()
 bundle=dict(x.split('=',1) for x in shlex.split((PRIVATE/'operator-client.env').read_text()))
 token=bundle['CB_MANUAL_OPERATOR_TOKEN']
 assert call('/api/v1/me',token)[0]==200
 values={'CB_PROD_MANUAL_OPERATOR_TOKEN':token}
 excluded=data('excluded')['excluded_job']
 job=submit(values,'2026-09-14')
 stages=[]
 for stage in ('processing','indexing'):
  selection=launch(['inspect',job,stage,'1'])['selection']
  result=launch(['execute',selection]);assert result['state']=='succeeded',result
 ...
 assert old['outbox_sent']==0 and all(s['attempts']==0 for s in old['stages'])
 assert proof['fts']['num_unindexed_rows']==0
 result={'actual_manual_inspect_execute':True,'stages':stages,'duplicate_execute_refused':True,
         'excluded_job':excluded,'proof':proof,'management_lease':False,
         'fresh_after_quiet_lease_refusal':True}
 atomic(ROOT/'manual-open-acceptance.json',json.dumps(result))
 return result
...
def paired(values,marker_name='paired-recovery'):
 # Invoked under management's live quiet lease: writer locks are held throughout.
 marker=ROOT/marker_name;marker.mkdir(mode=0o700)
 ...
 database='integration_restore' if marker_name=='paired-recovery' else 'integration_restore_postbrowser_v2'
 command(COMPOSE+['exec','-T','pg','createdb','-U','fixture',database])
 with (marker/'database.sql').open('rb') as incoming:
  p=subprocess.run(COMPOSE+['exec','-T','pg','psql','-U','fixture','-d',database,'-v','ON_ERROR_STOP=1'],stdin=incoming,capture_output=True,timeout=90)
 assert p.returncode==0
 return finish_pair(marker)

def post_browser(values):
 """Fresh post-interaction pair plus exact-image signing-key comparison."""
 from host import container,lib
 marker=ROOT/'paired-postbrowser-v2'
 assert not marker.exists()
 m=lib();m.ready();state=json.loads((PRIVATE/'credentials.json').read_text());token=m.login(state)
 assert m.call('GET','/api/v1/auths/',token=token)[0]==200
 result=paired(values,'paired-postbrowser-v2')
 restored=marker/'webui-restore/webui'
 assert restored.is_dir()
 import sqlite3,urllib.error,urllib.request,time,secrets
 db=sqlite3.connect('file:'+str(restored/'webui.db')+'?mode=ro',uri=True)
 chats=[r[0] for r in db.execute('select chat from chat')]
 assert any('Request034 synthetic recovery chat' in c and 'Request034 synthetic model response' in c for c in chats)
 assert any('Request034 synthetic attachment retention check' in c and 'browser-upload.txt' in c for c in chats)
 db.close()
 env=PRIVATE/'webui.env';original=env.read_text()
 assert 'ENABLE_SIGNUP=false\n' in original
 wrong=marker/'wrong-webui.env'
 wrong_key=secrets.token_urlsafe(48)
 wrong_env=''.join('WEBUI_SECRET_KEY='+wrong_key+'\n' if line.startswith('WEBUI_SECRET_KEY=') else line for line in original.splitlines(keepends=True))
 assert wrong_env!=original
 atomic(wrong,wrong_env)
 image=json.loads((PACKET/'descriptor.json').read_text())['webui_image']
 name='cbm-r034-webui-recovery'
 def probe(base,path,token=None,body=None):
 ...
 def one(key_env,directory):
  command(['docker','run','-d','--name',name,'--network','cbm-r034_default','--memory','1536m','--cpus','1.5','--cap-drop','ALL','--security-opt','no-new-privileges','--env-file',str(key_env),'--mount','type=bind,src='+str(directory)+',dst=/app/backend/data',image])
  try:
   row=json.loads(command(['docker','inspect',name]))[0]
   assert row['Image']==image
   base='http://'+row['NetworkSettings']['Networks']['cbm-r034_default']['IPAddress']+':8080'
   for _ in range(180):
    try:
     if probe(base,'/health')==200:break
    except (OSError,ValueError):pass
    time.sleep(2)
   else:raise RuntimeError('restored WebUI readiness timeout')
   auth=probe(base,'/api/v1/auths/',token=token)
   signup=probe(base,'/api/v1/auths/signup',body={'name':'denied','email':'denied@example.invalid','password':'invalid'})
   assert signup==403
   return auth
  finally:
   command(['docker','rm','-f',name])
 original_status=one(env,restored)
 assert original_status==200,original_status
 with tarfile.open(marker/'webui.tar') as tar:tar.extractall(marker/'wrong-restore',filter='data')
 wrong_status=one(wrong,marker/'wrong-restore/webui')
 assert wrong_status==401,wrong_status
 result.update(restored_chat_and_attachment=True,original_key_status=original_status,wrong_key_status=wrong_status,
               exact_webui_image=image,signup_disabled_in_both_restores=True,source_state_file_hashes_verified=True)
 return result
...
def finish_pair(marker):
 ...
 source=Path(next(m['Source'] for m in row['Mounts'] if m['Destination']=='/app/backend/data'))
 database='integration_restore' if marker.name=='paired-recovery' else 'integration_restore_postbrowser_v2'
 env=worker_environment();env['CB_DATABASE_URL']='postgresql+psycopg://fixture@pg/'+database
...
def auto_execute(values):
 ...
 old=next(j for j in before['jobs'] if j['meeting_id']=='request034-2026-09-11')
...
def operations(expected):
 ...
 return {'catalog-proof':catalog,'validate-boot':lambda _:boot_checks(expected),'setup-resume':resume_setup,'boot-enforce':lambda _:enforce(expected),'boot-reconcile':lambda _:reconcile(expected,Path('/proc/sys/kernel/random/boot_id').read_text().strip()),'validate-manual':manual,'validate-manual-open':manual_after_reconciliation,'validate-checkpoint':automatic,'validate-recovery':paired,'recovery-resume':lambda _:finish_pair(ROOT/'paired-recovery'),'validate-postbrowser-v2':post_browser,'validate-automatic':auto_execute,'validate-uncertain':uncertain,'data-inspect':lambda _:data()}
