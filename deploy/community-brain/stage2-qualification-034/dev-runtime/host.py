...
def address(service,port):
 ...
 net=row['NetworkSettings']['Networks'];assert set(net)=={'cbm-r034_default'}
 return 'http://'+net['cbm-r034_default']['IPAddress']+':'+str(port)
...
def environment(values):
 ...
 'CB_ENABLE_MODEL_CALLS':'false','CB_ENABLE_NETWORK_PUBLICATION':'false','CB_AUTOMATIC_PROCESSING':'true','CB_AUTOMATION_ROOT':'/state/automation-public',
 'CB_WEB_DIST':'/app/web/dist','CB_MEETING_ARCHIVE_ROOT':'/state/meeting-archive',
 'CB_MEETING_ARCHIVE_SHA256':hashlib.sha256((ROOT/'state/meeting-archive/manifest.json').read_bytes()).hexdigest(),
 'SSL_CERT_FILE':'/run/certs/ca-bundle.pem','PYTHONDONTWRITEBYTECODE':'1'}
 assert all("'" not in x and '\n' not in x for x in env.values())
 atomic(PRIVATE/'api.env',''.join(k+"='"+v+"'\n" for k,v in env.items()))

def holds():
 return {n:(hashlib.sha256((ROOT/'state'/n).read_bytes()).hexdigest() if (ROOT/'state'/n).exists() else None) for n in
...
def setup(values):
 ...
 from provisioning import private_files
 private_files(values)
 environment(values)
 credentials={'email':'request034@example.invalid','password':secrets.token_urlsafe(32)}
 ...
 'kuma':{'image':descriptor['monitor_images']['kuma'],'volumes':['kuma:/app/data'],'mem_limit':'256m','environment':{'UPTIME_KUMA_DB_TYPE':'sqlite'}},
 'nats':{'image':descriptor['nats_image'],'user':'0:0','read_only':True,'command':['-c','/config/server.conf'],'volumes':[str(PRIVATE/'nats')+':/config:ro','nats:/data'],'mem_limit':'128m'},
 })
 compose['volumes'].update(pg={},kuma={},nats={})
 ...
 command(COMPOSE+['up','-d','pg','provider','nats'])
 from provisioning import seed
 seed()
...
def install(values):
 ...
 m.api('POST','/api/v1/auths/signup',{'name':'Request034 development','email':state['email'],'password':state['password']})
 token=m.login(state)
 ...
 valves.update(retrieval_url=m.RETRIEVAL,api_key=values['CB_OPENWEBUI_RETRIEVAL_TOKEN'],top_k=1)
...
def accept(values):
 ...
 expected=json.loads((PACKET/'descriptor.json').read_text())
 row=json.loads(command(['docker','inspect',container('api')]))[0]
 actual=sorted([{'source':m['Source'],'target':m['Destination'],'read_only':not m['RW']} for m in row['Mounts'] if m['Type']=='bind'],key=lambda x:x['target'])
 assert actual==sorted(expected['api_mounts'],key=lambda x:x['target'])
 assert row['HostConfig']['Memory']==expected['api_memory']
 assert row['HostConfig']['NanoCpus']==expected['api_cpus']*1000000000
 assert row['HostConfig']['ReadonlyRootfs'] and row['Config']['Cmd'][1]=='-B'
 result.update(mounts=actual,memory_limit=row['HostConfig']['Memory'],container_id=row['Id'],nano_cpus=row['HostConfig']['NanoCpus'],read_only_root=row['HostConfig']['ReadonlyRootfs'],command=row['Config']['Cmd'])
 return result
...
def provider_upgrade(_):
 """Replace only the disposable synthetic provider after source verification."""
 marker=ROOT/'provider-upgrade-intent';assert not marker.exists();marker.touch(mode=0o600)
 current=json.loads((ROOT/'compose.json').read_text())
 previous=current['services']['provider']['volumes']
 assert previous==[DEV_ROOT+'/packet-v2/fixture:/source:ro']
 before={name:container(name) for name in ('api','webui','pg','nats','kuma','prometheus')}
 current['services']['provider']['volumes']=[str(PACKET/'fixture')+':/source:ro']
 atomic(ROOT/'compose.json',json.dumps(current,indent=2))
 command(COMPOSE+['up','-d','--no-deps','--force-recreate','provider'])
 import urllib.request
 for _ in range(30):
  try:
   with OPENER.open(address('provider',8999)+'/v1/models',timeout=3) as r:
    if r.status==200:break
  except (OSError,ValueError):pass
  time.sleep(1)
 else:raise RuntimeError('synthetic provider failed readiness')
 body={'model':'cbm-synthetic-model','messages':[{'role':'user','content':'synthetic'}],
       'stream':False}
 req=urllib.request.Request(address('provider',8999)+'/v1/chat/completions',
                            data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
 with OPENER.open(req,timeout=10) as response:
  value=json.load(response);assert value['choices'][0]['message']['content'].startswith('Request034 synthetic')
 assert before=={name:container(name) for name in before}
 return {'actual_synthetic_chat_provider':True,'source_packet_sha256':os.environ['CBM_PACKET_SHA256'],
         'api_webui_and_monitor_container_ids_unchanged':True,'external_provider_requests':0}

def disable_signup(_):
 """Seal the one-time synthetic enrollment; retain the existing WebUI volume."""
 marker=ROOT/'disable-signup-intent';assert not marker.exists();marker.touch(mode=0o600)
 env=PRIVATE/'webui.env';old=env.read_text()
 assert old.count('ENABLE_SIGNUP=true\n')==1
 before={name:container(name) for name in ('api','pg','nats','provider','kuma','prometheus')}
 atomic(env,old.replace('ENABLE_SIGNUP=true\n','ENABLE_SIGNUP=false\n'))
 command(COMPOSE+['up','-d','--no-deps','--force-recreate','webui'])
 import urllib.request
 for _ in range(120):
  try:
   with OPENER.open(address('webui',8080)+'/health',timeout=3) as response:
    if response.status==200:break
  except (OSError,ValueError):pass
  time.sleep(1)
 else:raise RuntimeError('WebUI readiness timeout after signup closure')
 req=urllib.request.Request(address('webui',8080)+'/api/v1/auths/signup',data=json.dumps({'name':'denied','email':'denied@example.invalid','password':'invalid'}).encode(),headers={'Content-Type':'application/json'})
 try:OPENER.open(req,timeout=10)
 except urllib.error.HTTPError as error:assert error.code==403,error.code
 else:raise AssertionError('signup unexpectedly enabled')
 assert before=={name:container(name) for name in before}
 state=json.loads((PRIVATE/'credentials.json').read_text());m=lib();assert m.login(state)
 return {'persisted_signup_disabled':True,'signup_http_status':403,'existing_synthetic_login':True,'other_dev_container_ids_unchanged':True}

def disable_signup_readback(_):
 assert (ROOT/'disable-signup-intent').exists()
 assert 'ENABLE_SIGNUP=false\n' in (PRIVATE/'webui.env').read_text()
 import urllib.request
 ...
 state=json.loads((PRIVATE/'credentials.json').read_text());assert lib().login(state)
 return {'persisted_signup_disabled':True,'signup_http_status':403,'existing_synthetic_login':True,'independent_readback_after_cold_start':True}
...
def main():
 ...
 os.environ['CBM_PACKET_SHA256']=expected
 ...
 operations={'worker-hold-probe':worker_hold_probe,'lock-probe':lock_probe,'controls':lambda x:controls(descriptor),'tick':lambda x:tick(expected),'boot-inspect':lambda x:boot_inspect(expected),'setup':setup,'install':install,'accept':accept,'tokens':verify_tokens,'revoked':lambda x:verify_tokens(x,True),'cache':cache,'webui-deliver':webui_deliver,'inspect':lambda x:inspect_runtime(),'provider-upgrade':provider_upgrade,'disable-signup':disable_signup,'disable-signup-readback':disable_signup_readback}
 if mode not in operations:
  from consumers import operations as consumer_operations
  from validation_host import operations as validation_operations
  operations.update(consumer_operations());operations.update(validation_operations(expected))
  from capacity_host import main as capacity_trial
  operations['capacity-trial']=capacity_trial
  from negative_host import run as negative_auth
  operations['negative-auth']=negative_auth
  operations['negative-auth-v2']=lambda values:negative_auth(values,'-v2')
