...
DEV_ROOT='/srv/dev-data/workspaces/cbm-successor-bindings-20260920-028'
...
def profile(name,revision="packet-v1"):
 ...
 queue={'url':'tls://nats:4222' if dev else 'tls://platform-events.patchoutech.lab:4223',
        'stream':'CBM_REQUEST028' if dev else 'COMMUNITY_BRAIN_PROD',
        'subject':'cbm.dev.request028.jobs.stage.ready.v1' if dev else 'cbm.prod.jobs.stage.ready.v1',
        'inbox':'_INBOX.cbm_request028_worker' if dev else '_INBOX.cbm_prod_worker',
        ...
        'authority_path':'/development/community-brain-dev/request028' if dev else '/applications/community-brain',
 ...
 return {'schema':2,'request_id':'CBM-SUCCESSOR-BINDINGS-20260920-028','profile':name,'revision':revision,
  ...
  'compose_project':'cbm-r028' if dev else 'community-brain-successor',
  'network':'cbm-r028_default' if dev else 'community-brain-production-staging_default',
  'api_container':'cbm-r028-api' if dev else 'community-brain-successor-api',
  'webui_container':'cbm-r028-webui' if dev else 'community-brain-successor-webui',
  ...
  'webui_data':'cbm-r028_webui' if dev else 'community-brain-successor-webui',
