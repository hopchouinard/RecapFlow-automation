...
def address(service,port):
 ...
 net=row['NetworkSettings']['Networks'];assert set(net)=={'cbm-r028_default'}
 return 'http://'+net['cbm-r028_default']['IPAddress']+':'+str(port)
...
def setup(values):
 ...
 environment(values)
 credentials={'email':'request028@example.invalid','password':secrets.token_urlsafe(32)}
...
def install(values):
 ...
 m.api('POST','/api/v1/auths/signup',{'name':'Request028 development','email':state['email'],'password':state['password']})
 token=m.login(state)
