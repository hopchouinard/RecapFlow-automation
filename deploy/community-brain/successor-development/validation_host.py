"""Bounded validation on disposable Request028 state. Never production."""
...
def submit(values,date,fathom=False):
 ...
 token=values['CB_PROD_MANUAL_OPERATOR_TOKEN'];meeting='request028-'+date
 sources={}
...
def finish_pair(marker):
 ...
 files={}
 for name in ('files','config','corpus','meeting-archive'):
  for original in (ROOT/'state'/name).rglob('*'):
   if original.is_file():
    relative=original.relative_to(ROOT/'state');other=restored/relative
    assert hashlib.sha256(original.read_bytes()).digest()==hashlib.sha256(other.read_bytes()).digest()
    files[str(relative)]=hashlib.sha256(other.read_bytes()).hexdigest()
 webui_files=0
...
def auto_execute(values):
 ...
 old=next(j for j in before['jobs'] if j['meeting_id']=='request028-2026-09-11')
