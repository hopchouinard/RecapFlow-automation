# Chunk Extraction Prompt (v3)

> **Version:** chunk-extraction-v3
> **Cloned from:** chunk-extraction-v2 (2026-05-03)
> **Targeted change:** the `has_unresolved_question` criterion is more permissive (defaults to `true` when in doubt; flags partial answers, deferred answers, and questions that get pivoted-away-from as unresolved). All other Stage C extractions (entities, speakers, decisions, action_items, topic_label, etc.) are unchanged from v2.
>
...
- `has_unresolved_question`: chunk contains a question (explicit or implied) that does NOT receive a clear, definitive answer within the same chunk. Set to `true` when any of the following apply:
  - A direct question gets only a partial answer (one part addressed, others ignored).
  - A question gets deferred ("I'll think about it", "let me get back to you", "we should table that").
  - The conversation pivots to another topic before the question is resolved.
  - The answer raises new questions instead of closing the original.
  - The question is acknowledged but not actually attempted.
  - An implied question (expressed uncertainty, doubt, or "I'm not sure how to...") goes unresolved.
  - Subtle cases: awkward pause, soft topic-change, or hedging without commitment after the question.

  Set to `false` ONLY when the chunk contains a question AND a clear, definitive answer addressing all parts of the question. **When in doubt, default to `true`** — flagging open questions over silent closure is the correct trade-off, since unresolved questions are a key signal of where the community is still figuring things out.
