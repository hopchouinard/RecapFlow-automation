...
- **`entities` field underpopulated.** Stage C extraction omits proper-noun entities that are clearly present in `full_text`. Surfaced during 2026-04-28 v2 validation: "Adam" appears in 6+ chunks' `full_text` across the live corpus, but in 0 chunks' `entities` arrays. v2 retrieval surfaces these chunks correctly via BM25 over `full_text`, so this is not a retrieval-layer issue — but it does mean the `entities` filter parameter on `/query` is less useful than the schema implies. Fix is a Stage C prompt revision (likely tightening the entity-extraction instruction + lowering the entity-significance threshold) plus a re-extraction pass with bumped `extraction_prompt_version`.
