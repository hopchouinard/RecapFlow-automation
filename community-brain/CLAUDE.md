...
- **Corpus-derived markers populated by `lint_corpus`.** Auto-triggered at end of `/ingest`. Atomic marker writes via `table.update()` so post-commit lint failures don't risk row loss. Operator can also run lint manually.
