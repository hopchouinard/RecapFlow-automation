...
This week's round-table covered enterprise AI deployment with members from Quebec, Australia, New Zealand, Mexico, Ohio, and California. Key topics included Microsoft's evolving AI stack with the new Anthropic partnership bringing Claude into Copilot, local LLM hosting via Proxmox, and Andrej Karpathy's LLM Wiki method. Wins included Ty Wells's user-driven development feedback loop, David's event-planning RFQ automation, Morgan Cook's cemetery management GIS project, and Elijah Stambaugh's victory in the Ohio state Presidential AI Challenge. Patrick previewed an upcoming open-source community project for collective digital intelligence, releasing within two weeks.

💡 KEY INSIGHTS

Layered model orchestration cuts costs immediately: use cheaper models like Codex for cleanup while reserving advanced models for complex reasoning.

Copilot Studio and Power Automate represent dead-end investments for production agents. For Microsoft shops needing longevity, Azure Foundry is the recommended path, especially with the Anthropic partnership embedding Claude across Excel, Word, PowerPoint, and desktop Copilot.

Google Enterprise vs. Pro shows a stark capability gap. Gemini Enterprise is currently described as unusable compared to the consumer tier, with kneecapped Notebook LM functionality.

Finding the right use case remains harder than building the solution. Ask clients what weekly tasks they lose the most time on, not what they think AI should do. Removing specific pain points creates immediate buy-in.

Meeting transcripts serve as goldmines for business analysis, revealing intent, challenges, and inefficiencies naturally.

Enterprise AI coding differs fundamentally from vibe coding. The SDLC is shifting toward Markdown-based development, with context sharing between Claude instances currently requiring human mediation.
...
Karpathy's LLM Wiki prompt functions as a RAG preprocessor, enriching metadata before chunking into vector databases.
...
A: Not yet deployed, but guidance is clear: Copilot Studio's drag-and-drop Power Automate approach is not the future paradigm. Azure Foundry is the better investment, with the Anthropic partnership potentially unlocking managed agents within the Microsoft ecosystem.
...
A: Do not ask what they think AI should do. Ask where they lose the most time every week—what tasks they cannot avoid but hate.
