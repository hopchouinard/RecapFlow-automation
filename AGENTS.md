...
Watches `./watch/` for Zoom chat logs and Fathom transcripts. Uses a **rendezvous pattern**: whichever file arrives second detects the partner file and triggers the pipeline. Runs a 4-step LLM prompt chain via OpenRouter (Codex Sonnet 4.6), saves outputs to `./output/<date>/`:
